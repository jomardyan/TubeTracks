...
class Config:
    ...
    def from_file(cls, config_path: Path) -> 'Config':
        ...
        if 'download' in parser:
            ...
            config.embed_metadata = section.getboolean('embed_metadata', config.embed_metadata)
            config.embed_thumbnail = section.getboolean('embed_thumbnail', config.embed_thumbnail)
    ...
    for config_path in CONFIG_LOCATIONS:
        if config_path.exists():
            try:
                ...
                console.print(f"[yellow]Warning:[/yellow] Failed to load config from {config_path}: {e}")
    return Config()
...
def log_download_result(result: 'DownloadResult'):
    ...
    if result.success:
        logging.info(f"SUCCESS: {result.url} -> {result.output_path} ({result.duration_seconds:.1f}s)")
    else:
        logging.error(f"FAILED: {result.url} - {result.error_code.value}: {result.error_message}")
...
def classify_error(exception: Exception) -> Tuple[ErrorCode, str]:
    ...
    if isinstance(exception, DownloadError):
        if 'urlopen' in error_str or 'connection' in error_str or 'timeout' in error_str:
    ...
    if any(kw in error_str for kw in ['timeout', 'connection', 'network', 'urlopen', 'socket']):
        return ErrorCode.NETWORK_ERROR, f"Network error: {exception}"
...
def run_preflight_checks(url: Optional[str], output_dir: str, quiet: bool = False) -> List[Tuple[str, bool, str]]:
    ...
    results.append(("Output Directory", dir_ok, dir_msg if dir_msg else f"Writable: {output_dir}"))
    
    # Check URL if provided
    if url:
        url_ok, url_msg = validate_url(url)
        results.append(("URL Format", url_ok, url_msg if url_msg else "Valid YouTube URL"))
    ...
    if not quiet:
        ...
        for check_name, passed, message in results:
            ...
            table.add_row(check_name, status, f"[{msg_style}]{message}[/{msg_style}]" if msg_style else message)
...
def download_audio(
    ...
    if cookies_file:
        if Path(cookies_file).exists():
            ...
            if not quiet:
                console.print(f"[yellow]Warning:[/yellow] Cookies file not found: {cookies_file}")
        ...
        try:
            if not quiet:
                if attempt == 1:
                    ...
                    console.print(f"[yellow]↻[/yellow] Retry attempt {attempt}/{max_retries}...")
            ...
            if last_error_code == ErrorCode.NETWORK_ERROR and attempt < max_retries:
                ...
                if not quiet:
                    console.print(f"[yellow]⚠[/yellow] Network error, retrying in {delay:.1f}s...")
                time.sleep(delay)
                continue
    ...
    for idx, url in enumerate(urls, 1):
        ...
        if not result.success:
            ...
            if max_failures > 0 and failure_count >= max_failures:
                if not quiet:
                    console.print(f"[yellow]Stopping: reached max failures ({max_failures})[/yellow]")
                break
...
def _print_batch_summary(results: List[DownloadResult], all_urls: List[str]):
    ...
    console.print(f"\n[bold cyan]═══════════════════════════════════════════[/bold cyan]")
    console.print(f"[bold cyan]           Batch Download Summary           [/bold cyan]")
    ...
    stats_table.add_row("Failed", f"[red]{len(failed)}[/red]" if failed else "[green]0[/green]")
...
Exit Codes:
  ...
  3 - All downloads failed
    ...
    parser.add_argument(
        '-b', '--batch-file',
        help='Download from batch file (one URL per line)'
    )
